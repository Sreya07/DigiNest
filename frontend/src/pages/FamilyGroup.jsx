  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={() => setModal("add")} className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-5 py-3 font-semibold text-white" type="button">
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        {members.map((member) => (
          <div key={member.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="grid h-14 w-14 place-items-center rounded-lg bg-blue-600 text-lg font-bold text-white">
              {member.relation[0]}
            </div>
            ...
            <button onClick={() => setModal("permission")} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-slate-100 px-4 py-2 font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200" type="button">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {["Birth Certificate", "Vaccination Record", "School Admission Document"].map((item) => (
            <div key={item} className="rounded-lg bg-blue-50 p-5 font-semibold text-blue-800 dark:bg-blue-950 dark:text-blue-200">{item}</div>
          ))}
        </div>
      </section>
          ...
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-2xl dark:bg-slate-900">
            ...
            {modal === "add" ? (
              <form onSubmit={addMember}>
                ...
                <input name="name" required className="mt-5 w-full rounded-lg border border-slate-200 px-4 py-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="Name" />
                <input name="relation" required className="mt-3 w-full rounded-lg border border-slate-200 px-4 py-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="Relation" />
                <button className="mt-5 rounded-lg bg-blue-600 px-5 py-3 font-semibold text-white" type="submit">Add member</button>
              </form>
            ) : (
              <div>
